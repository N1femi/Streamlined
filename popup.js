...
async function loadSettings() {
  ...
  document.getElementById("sessions-until-long-break").value =
    settings.sessionsUntilLongBreak
  document.getElementById("auto-start").checked = settings.autoStart
  document.getElementById("show-warning").checked =
  ...
  document.getElementById("show-tab-title").checked =
    settings.showTabTitle !== false
...
function updateDurationLabels(testMode) {
  ...
  document.querySelector('label[for="work-duration"]').textContent =
    `Work Duration (${unit}):`
  document.querySelector('label[for="break-duration"]').textContent =
    `Break Duration (${unit}):`
  document.querySelector('label[for="long-break-duration"]').textContent =
    `Long Break Duration (${unit}):`
}
  ...
  if (currentState.isRunning) {
    ...
    if (currentState.isPaused) {
      ...
      sessionType.textContent =
        (currentState.isWorkSession ? "Work" : "Break") + " Session (Paused)"
    } else {
      pauseBtn.disabled = false
      pauseBtn.textContent = "Pause"
      sessionType.textContent = currentState.isWorkSession
        ? "Work Session"
        : "Break Session"
    }
...
function setupEventListeners() {
  ...
  document
    .getElementById("setup-work-tabs-btn")
    .addEventListener("click", async () => {
  ...
  document
    .getElementById("setup-break-tabs-btn")
    .addEventListener("click", async () => {
  ...
  document
    ...
    .addEventListener("click", closeTabSelectionModal)
  document
    .getElementById("cancel-selection")
    .addEventListener("click", closeTabSelectionModal)
  document
    .getElementById("save-selection")
    .addEventListener("click", saveSelectedTabs)
  document
    .getElementById("select-all-tabs")
    .addEventListener("click", selectAllTabs)
  document
    .getElementById("deselect-all-tabs")
    .addEventListener("click", deselectAllTabs)
  document
    .getElementById("select-group-tabs")
    .addEventListener("click", selectAllActive)

  // Close modal when clicking outside
  document
    .getElementById("tab-selection-modal")
...
function renderTabsList(selectedUrls = []) {
  ...
  if (allTabs.length === 0) {
    tabsList.innerHTML =
      '<p style="text-align: center color: #666 padding: 20px">No tabs available</p>'
    return
  }
    ...
    checkbox.addEventListener("change", (e) => {
      if (e.target.checked) {
        ...
        checkboxes.forEach((cb) => {
          if (
            cb !== e.target &&
            cb.getAttribute("data-tab-url") === selectedUrl
          ) {
            cb.checked = false
          }
        })
...
async function saveSelectedTabs() {
  ...
  try {
    ...
    let group = currentState.tabGroups.find(
      (g) => g.type === currentTabSelectionType,
    )
    if (!group) {
